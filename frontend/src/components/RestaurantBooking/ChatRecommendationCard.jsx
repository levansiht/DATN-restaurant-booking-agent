  ...
  return (
    <article className="overflow-hidden rounded-[1.4rem] border border-[#e7d7bf] bg-[#fffaf3] shadow-[0_12px_28px_rgba(55,39,27,0.08)]">
      <div className="relative h-40 overflow-hidden bg-[#efe5d6]">
        ...
        />
        {item.image_badge ? (
          <span className="absolute left-3 top-3 rounded-full bg-[rgba(18,14,12,0.72)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#f7ead8]">
            {item.image_badge}
          </span>
        ) : null}
      </div>
        ...
        {item.tags?.length ? (
          <div className="flex flex-wrap gap-2">
            {item.tags.slice(0, 4).map((tag) => (
              <span
                key={`${item.id}-${tag}`}
                className="rounded-full border border-[#e6d6bc] bg-white px-3 py-1 text-[11px] font-medium text-[#735846]"
              >
                {tag}
              </span>
            ))}
